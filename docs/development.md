...
专项项目统一名为 `soluna-special`，不同业务使用自己的插件 ID。固定依赖 SDK v0.1.3；Health revision 为 `v1-draft-20260925`。生命周期与资源接口见 [special-protocol.md](special-protocol.md)。若业务包含多轮执行，Compiled 声明 Units / UnitTimeoutMs，每个 Run 处理当前 Unit，避免把 200 轮塞进一个不可观测的调用。
